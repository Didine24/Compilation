      ...
      switch(e->codop)
        ...
        case Ind: /* (tab_dim k of tau) x   int -> (tab_dim k-1 of tau) */
        {
          ...
          else if(tfg.DIM == 0)
          {
              ...
              typ_error("variable n'est pas un tableau",ligne);/* message erreur  */
          }
          else{ //error
              type_copy(&tp,terr);/* calcule le type                               */
              type_copy(&(e->typno),terr); /* affecte type                         */
              typ_error("acces sur args non-entiers",ligne);/* message erreur  */
          }
          return(tp);
        };
        case Pl:
        case Mo:
        case Mu: /* int x  int -> int */
        {
        ...
        if (type_eq(tfd,tint)){
          type_copy(&tp, e->typno);
            ...
            type_copy(&tp,terr);       /* tp := erreur                             */
            type_copy(&(e->typno),tp); /* affecte type                             */
            typ_error("taille tableau n'est pas un int",ligne);/* message erreur                */
        }
          return(tp);
        }
        case Af:                                                     /* affectation */
        ...
        case Wh:
        {
          ...
          if(type_eq(tfg,tboo)==0)   
          {
              type_copy(&(e->typno),terr);        /* affecte type                  */
              type_copy(&tp,terr);
              typ_error("condition non booleenne dans un Wh", ligne);
          }
          else if (!type_eq(tfd,tcom))  /* arg <> tcom   */
          {
              type_copy(&(e->typno),terr);                        /* affecte type  */
              type_copy(&tp,terr);
              typ_error("corps du Wh n'est pas une commande", ligne);
          }
          else
          {
              type_copy(&(e->typno),tcom);                         /* affecte type */
        ...
        default:
          return(tp);                            /* codop inconnu au bataillon */
        }/* fin switch          */
    }/* fin if (e!=NULL)         */
  else
